...
(async () => {
  ...
  const rawBody = data.description.replace(/\[\[SUMMARY:[\s\S]*?\]\]/g, '').trim();
  ...
  if (descError) { console.error(descError); } else { console.log('Updated 22956 successfully with SERVICE_ROLE_KEY!'); }
})();
