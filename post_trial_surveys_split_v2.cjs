...
async function postTrialSurveys() {
    console.log('--- Re-challenge: Corrected Posting Flow ---');
    
    // Using EXACT category strings from App.jsx: "ゲーム", "エンタメ", "グルメ"
    const surveyData = [
        {
            survey: {
    ...
    for (const item of surveyData) {
        ...
        console.log(`- Survey Created (ID: ${surveyId}, Category: ${item.survey.category})`);
        ...
        const { error: commentErr } = await supabase.from('comments').insert([{
            ...
            user_id: 'labi-system',
            user_name: 'Labi',
            content: "再挑戦らびっ！🐰 今度はカテゴリのアイコンもばっちり表示されているはず！✨ 2026・2027年の最新トレンドを楽しんでね！🥕💖"
        }]);
    ...
    console.log('--- Re-challenge Completed! ---');
}
